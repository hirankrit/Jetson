...
class StereoCameraNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("exposure_ms", 30)
        ...
        self.exposure_ms = self.get_parameter("exposure_ms").value
        ...
        self.left_pipeline = self.build_gstreamer_pipeline(self.left_sensor_id)
        self.right_pipeline = self.build_gstreamer_pipeline(self.right_sensor_id)

        self.get_logger().info(f"Left pipeline: {self.left_pipeline}")
        self.get_logger().info(f"Right pipeline: {self.right_pipeline}")

        # Open cameras (with delay to avoid resource conflict)
        import time

        self.get_logger().info("Opening left camera...")
        self.left_cap = cv2.VideoCapture(self.left_pipeline, cv2.CAP_GSTREAMER)

        # Wait for left camera to fully initialize
        time.sleep(2)

        self.get_logger().info("Opening right camera...")
        self.right_cap = cv2.VideoCapture(self.right_pipeline, cv2.CAP_GSTREAMER)
        ...
        self.get_logger().info(
            ...
            f"resolution={self.width}x{self.height}, fps={self.framerate}"
        )
        self.get_logger().info(
            "Note: Optimized settings for coverage (+81%) and stability"
        )
    ...
    def timer_callback(self):
        ...
        if not ret_left or not ret_right:
            self.get_logger().warn(
                "Failed to capture frames: " f"left={ret_left}, right={ret_right}"
            )
            return
        ...
        if self.frame_count % 30 == 0:
            self.get_logger().info(f"Published {self.frame_count} stereo frame pairs")
